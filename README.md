...
```text
├── data/
│   └── Merged_Annually_Quarterly.csv   # Raw source dataset
...
* **Live Database Integration:** The current React frontend is powered by exported static datasets. A future enhancement is to wire the frontend directly to the Supabase/PostgreSQL instance via a backend API (e.g., FastAPI) for real-time querying.
...
* **Test Coverage & CI/CD:** Replace boilerplate Playwright tests with actual end-to-end tests, set up GitHub Actions to automatically lint the SQL schema and run test queries on mock data.
